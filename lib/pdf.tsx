...
export function GeneratePdf() {
  ...
  const ventasCalculo = datosVentas.reduce<{ [key: string]: VentasCalculo }>(
    (acc, venta) => {
      ...
      // Manejo de gastos
      const gastosAcumulados = datosGastos.reduce<{
        [key: string]: GastoAcumulado;
      }>((gAcc, gasto) => {
        const gastoDate = new Date(gasto.updatedAt);
        const gastoMonthYear = `${gastoDate.toLocaleString("es-co", {
          month: "long",
      ...
      if (acc[monthYear]) {
        ...
        acc[monthYear].producto_costo +=
          (producto?.costo || 0) * venta.cantidad;
        acc[monthYear].total_ganancia +=
          venta.monto - (producto?.costo || 0) * venta.cantidad;
        acc[monthYear].total_gastos += gastosAcumulados[monthYear]?.total || 0;
      } else {
        acc[monthYear] = {
          ventas: [venta],
          total: venta.monto,
  ...
  const productosPorMes = Object.entries(
    datosProductos.reduce<Record<string, Producto[]>>(
  ...
  const gastosPorMes = Object.entries(
    datosGastos.reduce<Record<string, Gasto[]>>((acc, gasto: Gasto) => {
  ...
  const ventasPorMes = Object.entries(
    datosVentas.reduce<Record<string, Venta[]>>((acc, venta: Venta) => {
      const date = new Date(venta.updatedAt);
      const monthYear = `${date.toLocaleString("es-co", { month: "long" })}`;
      if (!acc[monthYear]) {
        acc[monthYear] = [];
  ...
  const pdfSubmit = async () => {
    ...
    autoTable(doc, {
      ...
      body: Object.entries(ventasCalculo).map(([monthYear, data]) => [
        ...
        data.total_cantidad,
        data.producto_costo.toLocaleString("es-CO", {
          style: "currency",
          currency: "COP",
        }),
        data.total_ganancia.toLocaleString("es-CO", {
          style: "currency",
          currency: "COP",
        }),
        data.total_gastos.toLocaleString("es-CO", {
          style: "currency",
          currency: "COP",
        }),
      ]),
    ...
    autoTable(doc, {
      ...
      body: ventasPorMes.flatMap(([month, ventas]) =>
        ventas.map((venta) => [
          datosProductos.find((p) => p.id === venta.productoId)?.nombre ||
            "Desconocido",
          venta.cantidad,
  ...
  return (
    <button
      onClick={pdfSubmit}
      className="bg-[#EE7890] text-white px-4 py-2  hover:bg-[#bd5167] absolute left-1 top-1"
    >
      {" "}
      Generar PDF de historial pdf
    </button>
  );
}
